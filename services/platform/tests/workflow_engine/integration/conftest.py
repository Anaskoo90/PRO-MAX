"""Integration test fixtures for Workflow Engine — same pattern as
tests/boards/integration/conftest.py: hits a real PostgreSQL instance
(migrated via `alembic upgrade head`, which now includes 0005's
`workflow_engine` schema), skipped automatically if unreachable."""
...
async def uow():
    ...
    async with WorkflowEngineUnitOfWork(session_factory) as unit_of_work:
        yield unit_of_work
