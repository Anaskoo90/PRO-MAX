"""Integration test fixtures for Boards & Agile Management — same pattern as
tests/tasks/integration/conftest.py: hits a real PostgreSQL instance
(migrated via `alembic upgrade head`, which now includes 0004's `boards`
...
async def uow():
    ...
    async with BoardsUnitOfWork(session_factory) as unit_of_work:
        yield unit_of_work
