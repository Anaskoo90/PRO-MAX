"""Outbox table + writer for the `ticket_system` schema (Outbox Pattern,
ADR-014) — identical mechanism to every other context's outbox, kept as
this context's own table per schema-per-bounded-context (ADR-001)."""
...
from app.ticket_system.infrastructure.orm_models import TicketSystemBase


class OutboxMessageOrmModel(TicketSystemBase):
    __tablename__ = "outbox_messages"
    __table_args__ = (
        Index("ix_ticket_system_outbox_messages_published_at", "published_at"),
