"""Tasks & Work Management's own append-only audit trail — same pattern as
Identity's AuditLogRecord and Projects' ProjectsAuditLogRecord, kept in
this context's own schema per ADR-001 (schema-per-bounded-context)."""
...
class TasksAuditEventCategory(StrEnum):
...
class TasksAuditLogRecord:
    def __init__(
        self, *, id: EntityId, org_id: OrgId, category: TasksAuditEventCategory, action: str,
    ...
    @classmethod
    def create(
        cls, *, org_id: OrgId, category: TasksAuditEventCategory, action: str, actor_user_id: UserId | None,
