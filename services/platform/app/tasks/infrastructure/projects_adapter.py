...
ProjectMemberSummary (application.ports).

Deliberately does NOT touch Projects' infrastructure/composition — only
the two service instances handed to it here, both already public
attributes on ProjectsModule (project_service, project_membership_service),
so nothing in Projects needs to change for this to exist.
"""
