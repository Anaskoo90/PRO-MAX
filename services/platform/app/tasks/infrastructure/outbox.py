"""Outbox table + writer for the `tasks` schema (Outbox Pattern, ADR-014) —
identical mechanism to Identity's/Projects' outbox, kept as this context's
own table per schema-per-bounded-context (ADR-001)."""
...
class OutboxMessageOrmModel(TasksBase):
    __tablename__ = "outbox_messages"
    __table_args__ = (
        Index("ix_tasks_outbox_messages_published_at", "published_at"),
        {"schema": "tasks"},
    )
