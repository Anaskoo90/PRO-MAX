"""Boards & Agile Management's own append-only audit trail — same pattern
...
class BoardsAuditEventCategory(StrEnum):
...
class BoardsAuditLogRecord:
    def __init__(
        self, *, id: EntityId, org_id: OrgId, category: BoardsAuditEventCategory, action: str,
    ...
    @classmethod
    def create(
        cls, *, org_id: OrgId, category: BoardsAuditEventCategory, action: str, actor_user_id: UserId | None,
        resource_type: str, resource_id: str, metadata: dict[str, Any] | None = None,
    ) -> "BoardsAuditLogRecord":
        return cls(
