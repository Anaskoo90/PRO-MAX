"""Outbox table + writer for the identity schema (Outbox Pattern, ADR-014).
A separate relay job (platform_core.jobs) reads unpublished rows and
forwards them to the RabbitMQ Event Bus — out of scope for this bootstrap,
flagged as the natural next piece once a bounded context actually needs to
publish across the bus."""
...
class OutboxMessageOrmModel(IdentityBase):
